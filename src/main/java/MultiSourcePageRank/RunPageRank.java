 ...
 */
public class RunPageRank extends Configured implements Tool {
  private static final Logger LOG = Logger.getLogger(RunPageRank.class);
  ...
  private static class MapClass extends
    ...
    public void setup(Context context){
      ...
      //modSrouce = context.getConfiguration().getStrings(SOURCES).split(",");
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      if (node.getAdjacenyList().size() > 0) {
        ...
        float[] m = new float[sourceNum];
        for(int i = 0; i < sourceNum; i++){
          m[i] = node.getPageRank().get(i) - (float) StrictMath.log(list.size());
        }
        ...
        for (int i = 0; i < list.size(); i++) {
          ...
          intermediateMass.setPageRank(new ArrayListOfFloatsWritable(m));
  ...
  private static class CombineClass extends
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> values, Context context)
      ...
      float[] massArray = new float[sourceNum];

      for(int i = 0; i < sourceNum; i++){
        massArray[i] = Float.NEGATIVE_INFINITY;;
      }
      ...
      for (PageRankNode n : values) {
        if (n.getType() == PageRankNode.Type.Structure) {
          ...
          for (int i = 0; i < massArray.length ; i++){
            massArray[i] = sumLogProbs(massArray[i], n.getPageRank().get(i) );
          }
      ...
      if (massMessages > 0) {
        ...
        intermediateMass.setType(PageRankNode.Type.Mass);
        intermediateMass.setPageRank(new ArrayListOfFloatsWritable(massArray));
  ...
  private static class ReduceClass extends
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
      ...
      node.setPageRank(new ArrayListOfFloatsWritable(massArray));
      ...
      if (structureReceived == 1) {
        ...
        float[] temp = new float[totalMass.length];
        for(int i = 0; i < totalMass.length; i++){
          temp[i]=sumLogProbs(totalMass[i], massArray[i]);
        }
        totalMass = temp;
  ...
  private static class MapPageRankMassDistributionClass extends
    ...
    private static String modSrouce[]; 
    private static int sourceNum;
    private String[] missingMassString;
    private float[] missingMassArray;
    private ArrayList<Integer> sourcesAL ;
    ...
    public void setup(Context context) throws IOException {
      ...
      missingMassString =  conf.getStrings("MissingMass");
      nodeCnt = conf.getInt("NodeCount", 0);

      inputSource =context.getConfiguration().get(SOURCES);
      modSrouce = inputSource.split(",");
      sourceNum = modSrouce.length;
      missingMassArray = new float[sourceNum];

      for(int i = 0; i < sourceNum; i++){
        missingMassArray[i] = Float.parseFloat( missingMassString[i]) ;
      }

      ///

      sourcesAL = new ArrayList<Integer>(sourceNum);
      for (int i =0; i < sourceNum; i++) {
        sourcesAL.add(i , Integer.parseInt(modSrouce[i]) );
      }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
        ...
        ArrayListOfFloatsWritable pList = node.getPageRank();
        float link;
        float jump;
        ...
        for(int i = 0; i < sourceNum; i++){
          ...
          float p = node.getPageRank().get(i);

          //if( node.getNodeId() == )
          if( node.getNodeId() == sourcesAL.get(i)  ){

            link = (float) Math.log(1.0f - ALPHA) + sumLogProbs(p, (float)Math.log(missingMassArray[i]));
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new RunPageRank(), args);
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    options.addOption(OptionBuilder.withArgName("sources").hasArg()
        .withDescription("# of sources").create(SOURCES));
    ...
    String inputSource = cmdline.getOptionValue(SOURCES);
    ...
    // Iterate PageRank.
    for (int i = s; i < e; i++) {
      iteratePageRank(i, i + 1, basePath, n, inputSource);
    }
  ...
  private void iteratePageRank(int i, int j, String basePath, int numNodes,
                                String inputSource) throws Exception {
      //boolean useCombiner, boolean useInMapperCombiner) throws Exception {
    
    // Each iteration consists of two phases (two MapReduce jobs).

    // Job 1: distribute PageRank mass along outgoing edges.
    float[] mass = phase1(i, j, basePath, numNodes, inputSource);
  ...
  private float[] phase1(int i, int j, String basePath, int numNodes,
   ...
   // job.setMapperClass(useInMapperCombiner ? MapWithInMapperCombiningClass.class : MapClass.class);
    ...
    //float mass = Float.NEGATIVE_INFINITY;
    float[] mass = new float[mod.length];
    float[] temp = new float[mod.length];

    for(int g = 0; g<mod.length; g++){
      mass[g] = Float.NEGATIVE_INFINITY;
    }
    ...
    for (FileStatus f : fs.listStatus(new Path(outm))) {
      ...
      for(int gg = 0; gg < mod.length; gg++){
        //mass = sumLogProbs(mass, fin.readFloat());
        temp[gg]= fin.readFloat();
      }

      //mass = sumLogProbs(mass,temp);


      float[] tempTwo = new float[mass.length];
      for(int gg = 0; gg < mass.length; gg++){
        tempTwo[gg]=sumLogProbs(mass[gg], temp[gg]);
      }
      mass = tempTwo;
  ...
  private void phase2(int i, int j, float[] missing, String basePath, int numNodes, String inputSource) throws Exception {
    Job job = Job.getInstance(getConf());
