 ...
 */
public class BuildPRRecords extends Configured implements Tool {
  private static final Logger LOG = Logger.getLogger(BuildPRRecords.class);
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNode> {
    ...
    private static List<Integer> sourceL;
    ...
    public void setup(Mapper<LongWritable, Text, IntWritable, PageRankNode>.Context context) {
      ...
      for(int i = 0; i < sourceNum ; i++) {
        sourceL.add(Integer.parseInt(modSrouce[i]));
       }
    }
    ...
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      float [] pages = new float[sourceNum];

      for (int i = 0; i<sourceL.size(); i++) {

        if(nid.get() == sourceL.get(i)){
          pages[i] = (float)StrictMath.log(1.0f);
        }else{
          pages[i] = (float)StrictMath.log(0.0f);
        }
      }
  ...
  public BuildPRRecords() {}

  private static final String INPUT = "input";
  private static final String OUTPUT = "output";
  private static final String NUM_NODES = "numNodes";
  private static final String SOURCES = "sources";
//  private static final String NUMOFSOURCE = "numofsources";
  ...
  public int run(String[] args) throws Exception {
    ...
    LOG.info("Tool name: " + BuildPRRecords.class.getSimpleName());
    ...
    conf.setStrings(SOURCES, inputSource );
//    conf.setInt(NUMOFSOURCE, modSources.length );

    Job job = Job.getInstance(conf);
    job.setJobName(BuildPRRecords.class.getSimpleName() + ":" + inputPath);
    job.setJarByClass(BuildPRRecords.class);
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new BuildPRRecords(), args);
  }
}
